...
#Hist Gradient boosting op merged_df
...
param_grid = {
    'max_depth': [6, 8, 10],  # max_depth is about how many levels the tree has,( 8 the best)
    'learning_rate': [0.5, 0.6, 0.7], #(0.6 best)
    'max_iter': [1300, 1500, 1700], #1500
}
...
grid_search = GridSearchCV(estimator=hgbc, param_grid=param_grid, cv=10, n_jobs=-1, scoring='roc_auc')
...
accuracy_scores = cross_val_score(hgbc_best, X_train, y_train, cv=kfold)
print("Accuracy: %0.2f (+/- %0.2f)" % (accuracy_scores.mean(), accuracy_scores.std() * 2))

auc_scores = cross_val_score(hgbc_best, X_train, y_train, cv=kfold, scoring='roc_auc')
print("AUC: %0.2f (+/- %0.2f)" % (auc_scores.mean(), auc_scores.std() * 2))

precision_scores = cross_val_score(hgbc_best, X_train, y_train, cv=kfold, scoring='precision')
print("Precision: %0.2f (+/- %0.2f)" % (precision_scores.mean(), precision_scores.std() * 2))

recall_scores = cross_val_score(hgbc_best, X_train, y_train, cv=kfold, scoring='recall')
print("Recall: %0.2f (+/- %0.2f)" % (recall_scores.mean(), recall_scores.std() * 2))

y_train_pred = hgbc_best.predict(X_train)
y_test_pred = hgbc_best.predict(X_test)
y_test_pred2 = hgbc.predict(X_test)

print("Train AUC: ", roc_auc_score(y_train, y_train_pred))
print("Test AUC: ", roc_auc_score(y_test, y_test_pred))

# Calculate accuracy, precision, and recall from hgbc_best
...
# Calculate accuracy, precision, and recall from hgbc
