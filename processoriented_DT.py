...
param_grid = {
    'max_depth': [8, 10, 12], # zelfde als xgboost om te kunnen vergelijken
...
grid_search = GridSearchCV(estimator=clf, param_grid=param_grid, cv=10, n_jobs=-1, scoring='roc_auc')
...
accuracy_scores = cross_val_score(clf, X_train, y_train, cv=kfold)
print("Accuracy: %0.2f (+/- %0.2f)" % (accuracy_scores.mean(), accuracy_scores.std() * 2))

auc_scores = cross_val_score(clf, X_train, y_train, cv=kfold, scoring='roc_auc')
print("AUC: %0.2f (+/- %0.2f)" % (auc_scores.mean(), auc_scores.std() * 2))

precision_scores = cross_val_score(clf, X_train, y_train, cv=kfold, scoring='precision')
print("Precision: %0.2f (+/- %0.2f)" % (precision_scores.mean(), precision_scores.std() * 2))

recall_scores = cross_val_score(clf, X_train, y_train, cv=kfold, scoring='recall')
...
# Evaluate the accuracy of the classifier adhv clf_best model
...
#adhv clf model
accuracy = balanced_accuracy_score(y_test, y_pred2)
precision = precision_score(y_test, y_pred2)
recall = recall_score(y_test, y_pred2, average='macro', zero_division=1)
AUC = roc_auc_score(y_test, y_pred2)
print(f"Accuracy2: {accuracy:.2f}")
print(f"Precision2: {precision:.2f}")
print(f"Recall2: {recall:.2f}")
print(f"AUC2: {AUC:.2f}")
