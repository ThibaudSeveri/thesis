...
#Logistic Regression op merged_df
...
#oversampling toepassen zodat we 80/20% hebben van non-leave en leave voor betere voorspelling using SMOTE, want imbalanced dataset
...
param_grid = {
    'penalty': ['l2'], #penalty type (L1 or L2)
    'C': [0.001, 0.005, 0.01, 0.05, 0.5, 1], # regularization strength (C)
    'max_iter': [500, 1000, 2000] #maximum aantal iterations
}
...
X_train_scaled = scaler.fit_transform(X_train) #fit_tranform omdat het hier de mean en sd berekend van z met z= (x-u)/s
X_test_scaled = scaler.transform(X_test) # hier geen fit omdat de test set de mean en sd niet mag weten om te kunnen evalueren
...
model_best = LogisticRegression(random_state=42,
                                C=grid_search.best_params_['C'],
                                penalty=grid_search.best_params_['penalty'],
                                max_iter=grid_search.best_params_['max_iter'])\

#de twee modellen fitten
model_best.fit(X_train_scaled, y_train)
model.fit(X_train_scaled, y_train)

# checks if model works
model.predict(X_train_scaled)
model.score(X_train_scaled, y_train) #percentage dat echt accuraat is
model.score(X_test_scaled, y_test) #percentage dat accuraat is van test set

#checks if model_best works
model_best.predict(X_train_scaled)
model_best.score(X_train_scaled, y_train) #percentage dat echt accuraat is
model_best.score(X_test_scaled, y_test) #percentage dat accuraat is van test set

#print intercept en slope van model_best
print(f"intercept:{model_best.intercept_}")
print(f"slope:{model_best.coef_}")

# perform 10-fold cross-validation and print the results for different metrics
kfold = KFold(n_splits=10, shuffle=True, random_state=42)
accuracy_scores = cross_val_score(model, X_train_scaled, y_train, cv=kfold)
...
precision_scores = cross_val_score(model, X_train_scaled, y_train, cv=kfold, scoring='precision')
...
#model outcome
print("Test AUC2: ", roc_auc_score(y_test, y_test_pred2))
